-- Chisel description
description = "Draw HTTP request latencies"
short_description = "HTTP latency"
category = "Network"
...
function on_transaction(transaction)
	...
	sliding_window[sliding_window_pos] = val
...
function on_interval(ts_s, ts_ns, delta)
	...
	local min_lat = format_time_interval(sorted_window[0])
	local maxIdx = 0 
	if sliding_window_size > 0 then 
		maxIdx = sliding_window_size - 1 
	end
	local max_lat = format_time_interval(sorted_window[maxIdx])
	
	terminal.clearline()
	io.write(string.format("RPS = %d\tp50 = %s\tp75 = %s\tp90 = %s\tp95 = %s\tp99 = %s\tp999 = %s\tmin = %s\tmax = %s\n", count * refresh_per_sec,  p50, p75, p90, p95, p99, p999, min_lat, max_lat))
